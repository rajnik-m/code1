Friend Class FDESuppressionDisplay
  ...
  Friend Overrides Sub RefreshContactData(ByVal pContactInfo As CDBNETCL.ContactInfo)
    ...
    Dim vControl As Control = FindControl(epl, "MailingSuppression")
    ...
    If mvDefaultSettings.Length > 0 Then
      ...
      If vParamList.ContainsKey("SuppressionGroup") Then vList("SuppressionGroup") = vParamList("SuppressionGroup")
    End If
    vList("Current") = "Y"
    Dim vDS As DataSet = DataHelper.GetContactData(CareServices.XMLContactDataSelectionTypes.xcdtContactSuppressions, pContactInfo.ContactNumber, vList)

    If vUseCombo Then
      Dim vDT As DataTable = DataHelper.GetTableFromDataSet(vDS)
      Dim vCombo As ComboBox = DirectCast(vControl, ComboBox)
      With vCombo
        .DisplayMember = "SuppressionDesc"
        .ValueMember = "SuppressionCode"
        .DataSource = vDT
      End With
    Else
