Partial Public Class UpdateTelephoneNumber
  ...
  Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    Try
      InitialiseControls(CareNetServices.WebControlTypes.wctUpdatePhoneNumber, tblDisplayData)
      BindDataGrid()
  ...
  Private Sub BindDataGrid()
    Try
      Dim vDGR As DataGrid = CType(Me.FindControl("TelephoneData"), DataGrid)
      ...
      vRestriction = "IsOrganisation = '' AND Email = 'N' AND WwwAddress = 'N'"
      Dim vDevices As String = GetDevices()
      If vDevices.Length > 0 Then vRestriction &= " AND DeviceCode IN ( " & vDevices & ")"
      If InitialParameters.ContainsKey("HideHistoricalPhoneFax") AndAlso InitialParameters("HideHistoricalPhoneFax").ToString = "Y" Then
  ...
  Public Overrides Sub HandleDataGridEdit(ByVal e As DataGridCommandEventArgs)
    ...
    If e.CommandName = If(InitialParameters.ContainsKey("HyperlinkText1"), InitialParameters("HyperlinkText1").ToString, "Edit") Then
      ...
      If Not vTable Is Nothing Then
        ...
        SetDropDownText("DiallingCode", vRow("DiallingCode").ToString)

        Dim vControl As Control = FindControlByName(tblDisplayData, "STDCode")
        If vControl IsNot Nothing Then
          SetDropDownText("STDCode", vRow("STDCode").ToString)
          SetTextBoxText("Number", vRow("Number").ToString)
        Else
          Dim vNumber As String = vRow("STDCode").ToString
          If vNumber.Length > 0 Then
            vNumber &= " " & vRow("Number").ToString
          Else
            vNumber = vRow("Number").ToString
          End If
          SetTextBoxText("Number", vNumber)
        End If

        If (String.Compare(vRow("ExDirectory").ToString, "Yes", False) = 0) Then
          SetCheckBoxChecked("ExDirectory")
        Else
          SetCheckBoxChecked("ExDirectory", False)
        End If
        SetTextBoxText("Extension", vRow("Extension").ToString)
  ...
  Public Overrides Sub ProcessButtonClickEvent(ByVal pValues As Object)
    Try
      ...
      Select Case vButtonID
        Case "Save", "Default"
          SaveTelephoneDetails(pValues.ToString)
  ...
  Private Sub SaveTelephoneDetails(Optional ByVal pSender As String = "")
    ...
    If (CType(ViewState("CommunicationNumber"), Integer) > 0) Then
      ...
      If Not vTable Is Nothing Then
        ...
        If (String.Compare(pSender, "Default", True) = 0) Then vParams("Default") = "Y"
